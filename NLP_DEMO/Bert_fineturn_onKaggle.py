...
def data(env):
    # 读json文件（以训练集为例）：
    path = '/kaggle/input/nlp-demo/KUAKE-QQR_train.json'

    if env == 'validation':
        path = '/kaggle/input/nlp-demo/KUAKE-QQR_dev.json'
    else:
        if env == 'test':
            path = '/kaggle/input/nlp-demo/KUAKE-QQR_test.json'
        else:
            path = '/kaggle/input/nlp-demo/KUAKE-QQR_train.json'
...
def generate_commit_xig(output_dir, task_name, test_dataset, preds: List[int]):
    record_id = test_dataset.record_id
    query1 = test_dataset.query1
    query2 = test_dataset.query2
    labels = test_dataset.labels

    pred_test_examples = []
    for idx in range(len(record_id)):
        label = preds[idx]
        pred_example = {'id': record_id[idx], 'query1': query1[idx], 'query2': query2[idx], 'label': preds[idx]}
        pred_test_examples.append(pred_example)
...
def main():
    ...
    record_id_validation, query1_validation, query2_validation, labels_validation = data('validation')
    record_id_test, query1_test, query2_test, labels_test = data('test')
    ...
    dev_dataset = Dataset(
        record_id_validation, query1_validation, query2_validation, labels_validation,
        tokenizer=tokenizer,
        max_length=data_args.max_length
    )
    ...
    model = BertForSequenceClassification.from_pretrained(best_model_dir, num_labels=3)
    ...
    #     generate_commit(training_args.output_dir, 'KUAKE-QQR', test_dataset, preds)

    generate_commit_xig(training_args.output_dir, 'KUAKE-QQR_epcho_10_warmup', test_dataset, preds)
...
if __name__ == '__main__':
    ...
    print('START TIME IS : ', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    # os.environ["CURL_CA_BUNDLE"] = "/etc/ssl/certs/ca-certificates.crt"
    main()
    print('END TIME IS : ', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))